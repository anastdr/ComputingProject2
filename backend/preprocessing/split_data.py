...
def preprocess_folder(dataset_dir, processed_data_dir):
    """
    Preprocess and augment all images in the given dataset folder.
    
    :param dataset_dir: Path to the folder containing the image dataset.
    :param processed_data_dir: Path to the folder where processed and augmented images will be saved.
    """
    # Make sure the processed directory exists
    os.makedirs(processed_data_dir, exist_ok=True)

    # Loop through all the classes in the dataset folder
    for class_name in os.listdir(dataset_dir):
        class_path = os.path.join(dataset_dir, class_name)
        if not os.path.isdir(class_path):
            continue

        # Create a directory for the class inside the processed data directory
        class_folder = os.path.join(processed_data_dir, class_name)
        os.makedirs(class_folder, exist_ok=True)
        ...
        for f in image_files:
            ...
            # Apply augmentations only for this image
            for aug_name, transform_list in AUGMENTATIONS.items():
                new_filename = f"{class_name}_{aug_name}_{f}"
                save_path = os.path.join(class_folder, new_filename)
...
if __name__ == "__main__":
    ...
    dataset_dir = Path(__file__).resolve().parent.parent.parent / 'proces_data'/'train_images'
    processed_data_dir = Path(__file__).resolve().parent.parent.parent / 'proces_data'/'train_images'
    preprocess_folder(str(dataset_dir), str(processed_data_dir))
